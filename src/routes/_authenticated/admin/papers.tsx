    ...
    <div className="space-y-6">
      <header className="flex items-end justify-between gap-4">
        <div>
          ...
          <h1 className="mt-2 font-display text-3xl font-semibold tracking-tight">Papers</h1>
            ...
            {STATUSES.map((opt) => (
              <button
                ...
                className={`rounded px-2.5 py-1 text-xs font-medium capitalize transition-colors ${
                  status === opt ? "bg-foreground text-background" : "text-muted-foreground hover:text-foreground"
                }`}
              >
                {opt}
              </button>
      ...
      {pageItems.length === 0 ? (
        <EmptyState label="No papers match the current filters." />
      ) : (
              ...
              {pageItems.map((p) => (
                <tr key={p.id} className="hover:bg-muted/30">
                  <td className="px-4 py-3">
                    <Link to="/admin/papers/$id" params={{ id: p.id }} className="font-medium hover:underline">
                      {p.title}
                    </Link>
